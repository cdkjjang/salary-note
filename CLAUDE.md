...
- 가이드 글 **18편**: `lib/guides-1.ts`~`guides-11.ts`, `lib/guides.ts`(집계)
- **2026-09-06 통합 (26 → 18편).** 애드센스가 "가치가 별로 없는 콘텐츠"로 두 번 반려했고,
  원인 중 하나가 같은 주제를 여러 편이 나눠 갖는 구조였다. 배경은 워크스페이스 CLAUDE.md 8장.
  - 같은 노트 안 5쌍을 합쳤다 — 실수령액·4대보험·최저임금·퇴직금·연차
  - **세 편은 다른 노트로 넘겼다.** 연말정산 → 세금노트, 실업급여·퇴사 후 건강보험 → 퇴사노트.
    급여노트는 **재직 중 급여**를 맡고, 퇴사 이후는 퇴사노트, 세금 정산은 세금노트가 맡는다.
    이 경계를 다시 흐리지 말 것.
  - **사라진 슬러그 8개는 `next.config.ts`의 `redirects()`가 301로 보낸다. 지우지 말 것.**
    노트 간 3건은 목적지가 다른 저장소에 있어 테스트로 확인이 안 된다 —
    **배포 후 실제 요청으로 200인지 확인할 것.**
  - `lib/guides.test.ts`가 재발을 막는다 — 본문 하한(1,500자, 공백 제외 실측), 슬러그·부제 중복,
    없는 글을 가리키는 `related`, cta가 실재하는 계산기를 가리키는지, 리다이렉트 정합성.
  - **최저임금 글에 새 슬러그를 만들지 말 것.** 해마다 새로 쓰면 같은 내용이 연도별로
...
- **최저임금**: 매년 8월 고시, 이듬해 1월 1일 시행. `lib/hourly.ts`의 `MIN_WAGE` 표에
  **연도별로 추가**하면 된다(2025: 10,030 / 2026: 10,320 / 2027: 10,700 — 2026.8.5 고시).
  ...
  - **화면 제목·설명에 연도를 적을 때는 `CURRENT_MIN_WAGE_YEAR`를 쓸 것.** 손으로
    "2026년"이라고 적으면 1월 1일에 제목만 낡아 계산 결과와 어긋난다.
    `app/calc/hourly/page.tsx`가 이 방식이고 `lib/calc-metadata.test.ts`가 고정한다.
